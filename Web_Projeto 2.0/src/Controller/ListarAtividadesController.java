package Controller;

import java.io.IOException;
import java.util.ArrayList;

import javax.servlet.RequestDispatcher;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import javax.servlet.http.HttpSession;

import Model.Atividade;
import Service.PesquisaServiceA;

/**
 * Servlet implementation class ListarClientesController
 */
@WebServlet("/listarAtividades.do")
public class ListarAtividadesController extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse
	 *      response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		request.setCharacterEncoding("UTF-8");
		String chave = request.getParameter("data[search]");
		String acao = request.getParameter("acao");
		PesquisaServiceA pesquisaA = new PesquisaServiceA();
		ArrayList<Atividade> listaAtividade = null;
		HttpSession session = request.getSession();
		if (acao.equals("buscarID")) {
			listaAtividade = pesquisaA.CarregarTemaId(1);
		}
		session.setAttribute("listaAtividade", listaAtividade);

		RequestDispatcher dispatcher = request.getRequestDispatcher("ListarAtividade.jsp");
		dispatcher.forward(request, response);
	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse
	 *      response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		doGet(request, response);
	}

}